配置 Google Tag Manager 进行销售 (sale) 跟踪
...
有两种方法可以使用 Google Tag Manager 跟踪销售事件：
...
此方法在用户完成购买后跳转到订单确认或成功页面时跟踪销售。这种方法更可靠，因为不太可能被广告拦截器拦截，并提供更高的数据准确性。
...
<script>
  (function () {
    ...
    // 仅在 customer ID、amount 和 clickId 存在时跟踪销售事件
    if (customerId && amount && clickId) {
      dubAnalytics.trackSale({
        eventName: "Purchase",
        customerExternalId: customerId,
        amount: parseInt(amount), // 金额（以分计）
        invoiceId: invoiceId || undefined,
        currency: "usd", // 根据需要自定义
        paymentProcessor: "stripe", // 根据需要自定义
        clickId: clickId,
      });
    }
  })();
...
> **重要提示**：确保将 `customer_id` 和 `amount` 查询参数传递给订单确认页面，以便将销售事件归因于正确的客户和购买行为。
...
- 添加如下条件：
  - **Page URL** 包含 `/order-confirmation`
  - 或 **Page Path** 等于 `/checkout/success`
  - 或任何匹配您订单确认页面的 URL 模式

将此代码命名为 "Dub Sales Tracking - Order Confirmation" 并保存。

### 选项 2：结账表单跟踪

此方法在用户完成网站上的结账表单时立即跟踪销售。请注意，由于广告拦截器和时间问题，此方法可能不太可靠。
...
> **重要提示**：您需要自定义 DOM 选择器 (`getElementById('customer_id')`, `getElementById('amount')` 等) 以匹配您实际的表单字段 ID，或根据您的网站结构使用不同的方法来捕捉结账数据。
...
- 添加条件以指定哪些表单应触发销售跟踪（例如结账表单）

将此代码命名为 "Dub Sales Tracking - Checkout Form" 并保存。
...
3. **测试您选择的跟踪方法**：
   - **对于选项 1 (订单确认页面)**：导航到带有查询参数的订单确认页面（例如 `?customer_id=123&amount=5000&invoice_id=inv_123`）
   - **对于选项 2 (结账表单)**：导航到带有结账表单的页面并完成测试购买
